...
from model.kyt_modules import MultiScale_TemporalModeling, PointWiseTCN, ST_GC
...
class MyEncodingBlock(nn.Module):
    def __init__(self, in_channels, out_channels, A, num_frame, stride=1, residual=True,flag=True):
        super(MyEncodingBlock, self).__init__()
        ###
        eta = 4
        num_scale = 4
        ...
        ###
        # if in_channels == 3:
        #     self.agcn = ST_GC(in_channels, out_channels, A)
        # else:
        #     self.agcn = SA_GC(in_channels, out_channels, A)
        ...
        if not residual:
            ...
            # self.residual2 = UnitTCN(in_channels, out_channels, kernel_size=1, stride=stride)
            self.residual1 = PointWiseTCN(in_channels, out_channels, stride=stride, groups=self.num_scale)

        if in_channels != out_channels:
            self.residual2 = PointWiseTCN(in_channels, out_channels, groups=self.num_scale)
        else:
            self.residual2 = lambda x: x
    ...
    def forward(self, x, attn=None):
        res = x
        if self.flag:
            x = self.agcn(x, attn)  ###包含了激活
        else:
            x=self.agcn(x)
            x=self.relu(x + self.residual2(res))
        x = self.tcn(x)
        # x = self.tcn(x)
        y = self.relu(x + self.residual1(res))
