...
from model.modules import import_class, bn_init, MyEncodingBlock


class MyfixGCN(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, in_channels=3,
        ...
        base_frame = 64
        # self.l0 = MyEncodingBlock(in_channels, base_channel, A, residual=False,num_frame=base_frame,flag=False)
        self.l1 = MyEncodingBlock(base_channel, base_channel, A, num_frame=base_frame)
        self.l2 = MyEncodingBlock(base_channel, base_channel, A, num_frame=base_frame)
        self.l3 = MyEncodingBlock(base_channel, base_channel, A, num_frame=base_frame)
        self.l4 = MyEncodingBlock(base_channel, base_channel * 2, A, num_frame=base_frame, stride=2)
        self.l5 = MyEncodingBlock(base_channel * 2, base_channel * 2, A, num_frame=base_frame // 2)
        self.l6 = MyEncodingBlock(base_channel * 2, base_channel * 2, A, num_frame=base_frame // 2)
        self.l7 = MyEncodingBlock(base_channel * 2, base_channel * 4, A, num_frame=base_frame // 2, stride=2)
        self.l8 = MyEncodingBlock(base_channel * 4, base_channel * 4, A, num_frame=base_frame // 4)
        self.l9 = MyEncodingBlock(base_channel * 4, base_channel * 4, A, num_frame=base_frame // 4)
    ...
    def get_A(self, graph, k):
        ...
        return torch.from_numpy(I - np.linalg.matrix_power(A_outward, k)).float()  ###求k次幂，表示通过k步可到达的关系，确保为float类型
